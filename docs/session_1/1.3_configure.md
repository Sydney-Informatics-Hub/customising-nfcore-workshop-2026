...
All configuration options, including workflow parameters, will have a default setting that is defined using the `nextflow.config` file in the workflow project directory.

There are also several  in the `nextflow.config` file that are used :

All nf-core pipelines come with several standard configuration files. The main `nextflow.config` file for the pipeline defines all of the default values for the workflow parameters. It also defines the various profiles that the pipeline supports, and uses several `includeConfig` statements to include additional `.config` files from the `conf/` folder. The use of multiple `.config` files helps to logically separate different configuration options, such as default and test configuration options. Some of these `.config` files are included by default, while others can be optionally activated via profiles:
...
!!! example "Exercise 1.3.7"
    ...
    ??? success "Solution"
        ...
        Create a custom `.config` file that contains your favourite month, e.g., October:
