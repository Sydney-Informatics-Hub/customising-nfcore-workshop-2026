...
For example, we can check the version of Nextflow that we're running with the `-v` flag:

```bash
nextflow -v
```

![](../assets/1.2_nextflow-v.png){width=50%}

We can also get a more verbose output with the `-version` flag:

```bash
nextflow -version
```

![](../assets/1.2_nextflow-version.png){width=50%}

One of the Nextflow commands that you will end up using the most is the `run` command, which actually executes a pipeline. The `run` command and all of the other Nextflow commands each come with their own set of options and flags that you can view by appending the `-help` option to that command:

```bash
nextflow run -help
```

## 1.2.2 Nextflow environment variables

Nextflow makes use of several [environment variables](https://docs.seqera.io/nextflow/config#environment-variables) to control some of the low-level aspects of how it runs. For most users, Nextflow and nf-core together will work out of the box without setting any environment variables. However, to improve reproducibility and to optimise your resources, you may benefit from setting some of these environment variables, which can be **exported** before running a workflow and will be interpreted by Nextflow.
...
!!! example "Exercise 1.2.2 :stopwatch: 3 mins"

    We have pre-loaded Singularity images for you in the folder `~/singularity_cache`. Inspect the folder, then set it as the Singularity cache directory for Nextflow by exporting the `NXF_SINGULARITY_CACHEDIR` environmental variable within your `~/.bashrc` file.

    First, inspect the Singularity cache directory that has been set up:

    ```bash
    ls ~/singularity_cache
    ```

    Inside you will see a long list of `*.img` files; these are the singularity images that Nextflow will look for when running the pipeline:

    ```console title="Output"
    bioconductor-tximeta-1.20.1--r43hdfd78af_0.img
    community-cr-prod.seqera.io-docker-registry-v2-blobs-sha256-1d-1d425b12748ce54c44c01a535a1ef5867a6e16cbf62c43151012e893444b1673-data.img
    ...
    community-cr-prod.seqera.io-docker-registry-v2-blobs-sha256-34-34e733a9ae16a27e80fe00f863ea1479c96416017f24a907996126283e7ecd4d-data.img
    ...
    community-cr-prod.seqera.io-docker-registry-v2-blobs-sha256-d2-d2ec9b73c6b92e99334c6500b1b622edaac316315ac1708f0b425df3131d0a83-data.img
    ...
    ```

    At the moment, Nextflow has no way of knowing where these are, so if you run a workflow now, it will download everything it needs into a `work/singularity` subdirectory of your working directory. Let's tell Nextflow where to find pre-downloaded images by setting the `NXF_SINGULARITY_CACHEDIR` environment variable. We set this in `~/.bashrc` so that the variable is set every time you log into your machine.

    Open your `~/.bashrc` file in VS Code:
    ...
    Save and close the `.bashrc` file (`Ctrl + S` (Windows/Linux) or `Cmd + S` (Mac); alternatively via menus: `File -> Save`), then run the command `source ~/.bashrc` in any active terminal sessions. Alternatively, you can close and re-open your session to cause the uddated `.bashrc` file to be applied to your session. You can check that it is set by running:
    ...
    Nextflow will now look for pre-downloaded Singularity images in the `~/singularity_cache` directory, and download any new images into that directory as well.
...
Similar to the `nextflow` command, the available commands and options for nf-core tools can be displayed using the `--help` option:

```bash
nf-core --help
```

![](../assets/1.2_nf-core-help.png)

You can also print out the version of nf-core you have installed with the `--version` flag:

```bash
nf-core --version
```

![](../assets/1.2_nf-core-version.png)
...
One very useful nf-core tools command is `nf-core pipelines download`. As the name suggests, it downloads pipelines, but it also has some handy built-in features to help you run the pipelines too. Sometimes you may need to execute an nf-core workflow on a computer with no internet connection, for example if you have highly protected data. In this case, you will need to fetch the workflow files and manually transfer them to your offline system. The `nf-core pipelines download` command makes this process easier by additionally:

- Retrieving a specific version of the code of your choosing
- Downloading the nf-core community configs
- Downloading all of the required Singularity or Docker container images (and ensuring they are all correctly versioned)
- Optionally compressing the workflow into a single file (e.g. `.zip` or `.tar.gz`) for easy transfer to the offline system

If run without any arguments, the `nf-core pipelines download` will interactively prompt you for the required information, including:
...
Alternatively, you can supply all of the above information with the command line options, in which case the command will run without any interactive prompt. As usual, you can find information on all of the supported flags with the `--help` option:

```bash
nf-core pipelines download --help
```

![](../assets/1.2_downloadhelp.png){width=100%}

**Note** that the command line method gives you a few additional options that aren't currently supported by the interactive prompt, including the ability to download all of the [nf-core community configs](https://nf-co.re/configs) via the option `--download-configuration yes`.

!!! example "Exercise 1.2.3.2 :stopwatch: 5 mins"

    Let's download nf-core's `demo` pipeline using the `nf-core pipelines download` command. Specifically, let's:

    - Pull the `1.1.0` version of the pipeline
    - Download it into a folder called `demo/`
    ...
    Consulting the output of the `--help` command above, we can see that the arguments we want are:

    - `--revision 1.1.0`: this pulls the specific version we want
    - `--outdir demo`: this downloads the pipeline into a folder called `demo/`
    ...
    ```bash
    nf-core pipelines download demo --revision 1.1.0 --outdir demo --download-configuration yes --container-system none --compress none
    ```

    Run this command, and after a few seconds it should complete. You should see a new directory where you ran the command:
    ...
    demo/1_1_0:
    total 128K
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 assets
    -rwxrwxr-x 1 user1 user1 3.5K May 14 03:28 CHANGELOG.md
    -rwxrwxr-x 1 user1 user1 2.8K May 14 03:28 CITATIONS.md
    -rwxrwxr-x 1 user1 user1  14K May 14 03:28 CODE_OF_CONDUCT.md
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 conf
    drwxrwxr-x 3 user1 user1 4.0K May 14 03:28 docs
    -rwxrwxr-x 1 user1 user1 1.1K May 14 03:28 LICENSE
    -rwxrwxr-x 1 user1 user1 2.9K May 14 03:28 main.nf
    drwxrwxr-x 3 user1 user1 4.0K May 14 03:28 modules
    -rwxrwxr-x 1 user1 user1 1.8K May 14 03:28 modules.json
    -rwxrwxr-x 1 user1 user1  11K May 14 03:28 nextflow.config
    -rwxrwxr-x 1 user1 user1  15K May 14 03:28 nextflow_schema.json
    -rwxrwxr-x 1 user1 user1  812 May 14 03:28 nf-test.config
    -rwxrwxr-x 1 user1 user1 6.8K May 14 03:28 README.md
    -rwxrwxr-x 1 user1 user1  17K May 14 03:28 ro-crate-metadata.json
    drwxrwxr-x 4 user1 user1 4.0K May 14 03:28 subworkflows
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 tests
    -rwxrwxr-x 1 user1 user1  165 May 14 03:28 tower.yml
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 workflows

    demo/configs:
    total 68K
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 bin
    -rwxrwxr-x 1 user1 user1 1.6K May 14 03:28 CITATION.cff
    drwxrwxr-x 5 user1 user1 4.0K May 14 03:28 conf
    -rwxrwxr-x 1 user1 user1  273 May 14 03:28 configtest.nf
    drwxrwxr-x 4 user1 user1 4.0K May 14 03:28 docs
    -rwxrwxr-x 1 user1 user1 1.1K May 14 03:28 LICENSE
    -rwxrwxr-x 1 user1 user1   69 May 14 03:28 nextflow.config
    -rwxrwxr-x 1 user1 user1  15K May 14 03:28 nfcore_custom.config
    drwxrwxr-x 2 user1 user1 4.0K May 14 03:28 pipeline
    -rwxrwxr-x 1 user1 user1  17K May 14 03:28 README.md
    ```

    We can see that the `1_1_0` folder contains the pipeline code, including its `main.nf` file, `nextflow.config` file, its `modules/` and `subworkflows/` folders, and its configuration folder `conf/`.

    Meanwhile the `configs` folder is where the nf-core community configs were downloaded. The config files themselves are under the `conf/` directory:

    ```bash
    ls -1 demo/configs/conf
    ```
    ...
    The pipeline code is set up to find these and include them when you request the appropriate profile; for example, if you run the pipeline with `-profile nci_gadi`, it will find the config file stored at `demo/configs/conf/nci_gadi.config` and include it in the pipeline's configuration.

## 1.2.4 Downloading and executing workflows with `nextflow`

The `nextflow` command itself can also be used to download pipelines. Nextflow seamlessly integrates with code repositories such as [GitHub](https://github.com/), allowing you to use public Nextflow workflows &mdash; including nf-core workflows &mdash; quickly, consistently, and transparently.
...
Once the workflow is donwloaded, the Nextflow `run` command is used to initiate the execution of a workflow. If you ran the `nextflow pull` command, you would run:

```bash
nextflow run foo/bar
```

Alternatively, if you used `nextflow clone` to download the workflow into your current directory, the workflow files will be in a sub-folder called `bar` and so you would run:
...
!!! example "Exercise 1.2.4 :stopwatch: 2 mins"

    Use `nextflow clone` to download the [`nextflow-io/hello` Nextflow repository](https://github.com/nextflow-io/hello) to your local directory, then execute it.
...
You can also use the following one-liner to get the fields from the **last** run:

```bash
nextflow log $(nextflow log -q | tail -n 1) -f process,hash,duration
```
...
!!! example "Exercise 1.2.5 :stopwatch: 2 mins"

    Use the `log` command to view the following fields for each task run during your most recent Nextflow execution:

    - Process name
    - Task hash (i.e. the unique ID assigned to the task)
    - Task script

    First, run the `nextflow log` command without any additional arguments to get a list of you recent executions:
    ...
    2026-05-14 03:43:10     2.1s            chaotic_brahmagupta     OK      538ce50984      dfc2fbfc-c2df-4ff6-abca-e06a766480e9    nextflow run hello
    ```

    Next, review the list of log fields that are available to us:
    
    ```bash
    nextflow log -l
    ```

    ```console title="Output" hl_lines="11 23 30"
    attempt
    complete
    container
    cpu_model
    ...
    The fields we're interested in right now are `process`, `hash`, and `script`.

    Now, use the `-f` option to query these fields for the most recent run. In this example, the run name is `chaotic_brahmagupta`; remember that yours will be different:
...
!!! note "The Nextflow cache can be sensitive!"
    ...
    Sometimes, you can run into issues where the cache is **invalidated** and a re-run of a process is forced even when nothing seems to have changed. Often this will happen on some HPC systems where file modification times aren't synchronised perfectly across parallel file systems. In these cases, it can help to apply the `process.cache = 'lenient'` configuration option to tell Nextflow to only use the file name and size, but not the modification time, to determine whether the cache is valid or not.
