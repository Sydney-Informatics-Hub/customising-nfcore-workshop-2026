...
!!! example "Exercise 1.2.3.2 :stopwatch: 5 mins"
    ...
    ```bash
    nf-core pipelines download demo --revision 1.1.0 --download-configuration yes --container-system none --compress none
    ```
    ...
    ```bash
    ls -lh nf-core-demo_1.1.0/*
    ```

    ```console title="Output"
    nf-core-demo_1.1.0/1_1_0:
    total 128K
    ...
    ```bash
    ls -1 nf-core-demo_1.1.0/configs/conf
    ```
    ...
    The pipeline code is set up to find these and include them when you request the appropriate profile; for example, if you run the pipeline with `-profile nci_gadi`, it will find the config file stored at `nf-core-demo_1.1.0/configs/conf/nci_gadi.config` and include it in the pipeline's configuration.
