...
- [![](../assets/1.1_slack.png)](https://nfcore.slack.com/)

- [![](../assets/1.1_github.png)](https://github.com/nf-core)

- [![](../assets/1.1_bluesky.svg)](https://bsky.app/profile/nf-co.re)

- [![](../assets/1.1_linkedin.svg){width=200%}](https://www.linkedin.com/company/nf-core)
...
The **nf-core Slack** is one of the primary resources for nf-core users. There are dedicated channels for all workflows as well as channels for common topics. If you are unsure of where to ask questions, the `#help` and `#nostupidquestions` channels are a great place to start.

Joining multiple nf-core and Nextflow channels is important to keep up to date with the latest community developments and updates. In particular, [joining the nf-core Slack organisation](https://nf-co.re/join/slack) will keep you up-to-date with community announcements. If you are looking for more information about a workflow, the [nf-core YouTube channel](https://www.youtube.com/c/nf-core) regularly shares ByteSize seminars about best practises, workflows, and community developments.
...
Upcoming events are listed on the [nf-core event page](https://nf-co.re/events) and announced on [Slack](https://nfcore.slack.com/).

## 1.1.2 What is Nextflow?

![](../assets/1.1_nextflow.svg){width=100%}
...
!!! tip "Questions about Nextflow"
    ...
    If you have questions about Nextflow and deployments that are not related to nf-core you can ask them on the [Seqera Community forum](https://community.seqera.io/), as well as the [Nextflow Slack](https://nextflow.slack.com). It's worthwhile joining both the [nf-core](https://nf-co.re/join/slack) and [Nextflow](https://www.nextflow.io/slack-invite.html) Slack organisations and browsing the channels to get an idea of what types of questions are being asked on each channel. Searching channels can also be a great source of information as your question may have been asked before.

## 1.1.3 nf-core workflow structure

nf-core workflows follow a set of best practices and standardised conventions. nf-core workflows start from a **common template** and follow the same structure. Although you won’t need to edit code in the workflow project directory, having a basic understanding of the project structure and some core terminology will help you understand how to configure its execution.

![](../assets/1.1_structure.excalidraw.png){width=50%}

The most important parts of the typical nf-core structure to know about are:

- `main.nf`: This is the **entrypoint** for the pipeline; it's where Nextflow starts when you execute `nextflow run`. In nf-core pipelines, this file is typically dedicated to getting things set up, such as ensuring the pipeline parameters are set appropriately and running some housekeeping functions (e.g. the print-out of the pipeline parameters).
- `workflows/<workflow_name>.nf`: This is where the actual workflow is defined. The workflow in this file gets called by `main.nf` once all of the initial setup steps have run.
    - **Note** that for some workflows (including the `rnaseq` pipeline we will look at later in this workshop), the main workflow file has been changed to `workflows/<workflow_name>/main.nf`. This seems to be a recent change to the nf-core structure and currently is inconsistent between pipelines.
- `subworkflows/`: This directory holds **subworkflow** definitions: groups of modules that are used in combination with each other and have a common purpose. Subworkflows improve workflow readability and help with the reuse of modules within a workflow and between different workflows.
    - For example, the [`SAMTOOLS_STATS`](https://github.com/nf-core/modules/blob/master/modules/nf-core/samtools/stats/main.nf), [`SAMTOOLS_IDXSTATS`](https://github.com/nf-core/modules/blob/master/modules/nf-core/samtools/faidx/main.nf), and [`SAMTOOLS_FLAGSTAT`](https://github.com/nf-core/modules/blob/master/modules/nf-core/samtools/flagstat/main.nf) modules are all included in the [`BAM_STATS_SAMTOOLS`](https://github.com/nf-core/modules/blob/master/subworkflows/nf-core/bam_stats_samtools/main.nf) subworkflow.
    - Within an nf-core workflow, you will find a mix of both **nf-core subworkflows** that were developed by the community and shared in the [nf-core subworkflows GitHub repository](https://github.com/nf-core/modules/tree/master/subworkflows/nf-core), and **local subworkflows** that are specific to just that one pipeline.
- `modules/`: This directory holds **module** definitions. "Module" is nf-core's term for a Nextfow **process**, with the name reflecting the ability for modules published to nf-core's GitHub organisation to be reused between pipelines.
    - Modules specify the [inputs](https://docs.seqera.io/nextflow/process#inputs), [outputs](https://docs.seqera.io/nextflow/process#outputs), [directives](https://docs.seqera.io/nextflow/process#directives), and [script](https://docs.seqera.io/nextflow/process#script) for running a given process.
    - Most modules will execute a single tool in the script block and will make use of the directives, inputs, and outputs dynamically.
    - Similar to subworkflows, modules can also be developed and shared in the [nf-core modules GitHub repository](https://github.com/nf-core/modules/tree/master/modules/nf-core) or stored as a local module. All modules from the nf-core repository are version controlled and tested to ensure reproducibility.

![](../assets/1.1_worksubmod.excalidraw.png)

Breaking up a pipeline into a modular hierarchy of subworkflows and module rather than creating a single, monolithic script helps to keep the underlying code for nf-core pipelines manageable and makes it easier to test and maintain the code. However, it can be a little overwhelming when you first dive into the code base!
