...
It appears to have sometihng to do with `/tmp`. The samtools error shows that the piped command received no output, so the aligner didn't process any reads. This means our input data is likely not the cause.
...
As introduced in [Lesson 1.3.6](../session_1/1.3_configure.md/#configuring-processes), nf-core modules define an `ext.args` process directive that can be used to add any tool parameter to a process command. 
