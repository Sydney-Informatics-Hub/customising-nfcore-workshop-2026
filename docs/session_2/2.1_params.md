...
In this section we will focus on customising nf-core workflows with **workflow parameters** introduced in [Lesson 1.3.2](../session_1/1.3_configure.md/#132-workflow-parameters). 
...
In [Lesson 1.3.2](../session_1/1.3_configure.md/#132-workflow-parameters) we used a pipeline `--help` flag to view available parameters. This can be a handy quick check for available workflow parameters but does not provide as much detail as the nf-core user guides.
...
!!! example "Exercise 2.1.4.3 :stopwatch: 5 mins"
    Using the same method to find the process work directory with `nextflow log` and view the `.command.sh` file with `cat` or `more`.
...
!!! example "Exercise 2.1.5.1 :stopwatch: 5 mins"
    ...
    - Save the new run script, then submit your updated run with the command `bash run_rnaseq.sh`
