...
In this section we will focus on customising nf-core workflows with **pipeline parameters** introduced in [Lesson 1.3.2](../session_1/1.3_configure.md/#132-workflow-parameters). 
...
Each nf-core pipeline has a comprehensive online user guide including a detailed parameters guide. The parameters guide can be found at `nf-co.re/<pipeline>/<version>/parameters`, for example [nf-co.re/rnaseq/3.23.0/parameters](https://nf-co.re/rnaseq/3.23.0/parameters).

In [Lesson 1.3.2](../session_1/1.3_configure.md/#132-workflow-parameters) we used a pipeline `--help` flag to view available parameters. This can be a handy quick check for available pipeline parameters but does not provide as much detail as the nf-core user guides.

!!! example "Exercise 2.1.2 :stopwatch: 3 mins"
...
If you have never run the pipeline before, you won't have the benefit of cached task details in the `work` directory to view the task command, so you would need to manually inspect the code to figure out the full command. In this lesson, we will check the task command via the `work` directory. In [Lesson 2.2.2.3](2.2_config.md/#2223-discovering-default-process-settings) we will practice finding a process definition file within the nf-core codebase. 

<br>

Let's consider the example of quantification with the tool Salmon, part of the default pipeline run we executed in [Lesson 1.4.3](../session_1/1.4_rnaseq.md#143-run-the-pipeline). If you were to view the Nextflow code for this process (SALMON_QUANT), you would see the command within the Salmon process definition (`modules/nf-core/salmon/quant/main.nf`) file shown on the left of the diagram below:
...
!!! example "Exercise 2.1.3 :stopwatch: 6 mins"
...
!!! example "Exercise 2.1.4.1 :stopwatch: 2 mins"
...
```console title="&lt;workdir&gt;/.command.sh"
#!/usr/bin/env bash -C -e -u -o pipefail
salmon quant \
    --geneMap mm10_chr18.filtered.gtf \
    --threads 2 \
...
!!! example "Exercise 2.1.4.3 :stopwatch: 5 mins"
...
!!! example "Exercise 2.1.4.4 :stopwatch: 1 mins"
...
In [Lesson 1.4.4](../session_1/1.4_rnaseq.md/#144-setting-resource-limits), you entered the **full run command including all custom parameters directly on the command line**. This is handy for a quick test run, but run commands can get really long, making them error prone, and create difficulty keeping track of what parameters have been applied.
...
!!! example "Exercise 2.1.5.1 :stopwatch: 5 mins"
...
!!! example "Exercise 2.1.5.2 :stopwatch: 5 mins"
    ...
    ??? success "Solution"
        ...
        *Tip: Aligning the values is optional but can aid readability* 
        ...
        ```bash title="run_rnaseq_with_yaml.sh"
        #!/bin/bash
...
!!! example "Exercise 2.1.6.1 :stopwatch: 2 mins"
    ...
    !!! tip 
        The report as well as the `execution_trace_<date>_<time>.txt` file can be used to identify compute resources used by each process, helping to fine-tune resource configurations when building an institutional config for your compute platform. We will do this within [Lesson 2.2.5](2.2_config.md/#2251-customising-with-process-labels). 
...
!!! example "Exercise 2.1.6.2 :stopwatch: 2 mins"
...
The first is the MultiQC report, which we viewed in [Lesson 2.1.4](./#214-resolve-a-pipeline-warning-with-a-parameter). Software details can be found at the end of the report under the following headings: 
