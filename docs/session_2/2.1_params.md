...
!!! example "Exercise 2.1.1" :stopwatch: 3 mins
    ...
    ??? success "Solution"
        ...
        The [nf-core/rnaseq parameters user guide](https://nf-co.re/rnaseq/3.24.0/parameters) provides five pieces of information about the parameter:
...
!!! example "Exercise 2.1.3.2" :stopwatch: 5 mins
    Use the [nf-core/rnaseq parameters guide](https://nf-co.re/rnaseq/3.24.0/parameters/) to find the parameter that can correct our strandedness issue, and identify the argument to provide for our data.
...
!!! example "Exercise 2.1.4.1" :stopwatch: 5 mins
    ...
    Additionally, search the [nf-core/rnaseq parameters guide](https://nf-co.re/rnaseq/3.24.0/parameters) for the parameters that control saving trimmed fastq and unaligned reads, and add these parameters to the run script. 
...
POLL QUESTION: Which of the changes that we made do you think will cause 100% of the tasks to be re-run (not cached?) [ANSWER - edit samplesheet]

This will take ~ 5.5 minutes to run. While that is running, we will move on to set up the parameters file. 
...
!!! example "Exercise 2.1.4.2" :stopwatch: 5 mins
    ...
    If you are feeling adventurous, search the [nf-core/rnaseq parameters guide](https://nf-co.re/rnaseq/3.24.0/parameters) for any other parameters that you may wish to try. 
...
POLL TODO: Which do you prefer: 1 - run script alone, 2 - params file + run script


## 2.1.5 Hardcoded reproducibility lives within pipeline info! 
